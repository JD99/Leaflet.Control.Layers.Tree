...
;(function (L) {
  ...
  L.Control.Layers.Tree = L.Control.Layers.extend({
    ...
    _iterateTreeLayout: function (
      ...
      var checks_parrent = checks_parrent || false
      if (!tree) return
      ...
      var entry = creator("span", "checkbox-wrapper")
      var closed = creator(
        "span",
        this.cls.closed,
        sel,
      ...
      if (tree.children) {
        ...
        $(sensible)
          .find("span.leaflet-layerstree-opened")
          .parent("span")
        ...
        tree.children.forEach(function (child) {
          ...
          this._iterateTreeLayout(
            ...
            overlay,
            false,
            tree.checked === false ? false : true,
          )
          if (child.hide)
            $(node)
              .children(".leaflet-layerstree-children")
              .addClass(this.cls.hide)
          //L.DomUtil.addClass(child, this.cls.hide);
        }, this)
      } else {
      ...
      var label = creator(tree.layer ? "label" : "label", this.cls.label, entry)
      if (tree.layer) {
        // now create the element like in _addItem
        //console.log(checks_parrent)
        var checked = this._map.hasLayer(tree.layer)
        if (!checks_parrent) checked = false
        else checked = tree.checked === false ? false : true
        var input
        ...
        if (radioGroup) {
          ...
          $(entry).removeClass("checkbox-wrapper")
          $(input).addClass("magic-radio")
        } else {
          input = this._createCheckboxElement(checked)
          $(input).addClass("magic-checkbox")
        }
        if (this._layerControlInputs) {
        ...
        input = this._createCheckboxElementGroup(checked)
        $(input).addClass("magic-checkbox")
        entry.appendChild(input)
        $(entry)
          .find("input")
          .change(function () {
            if ($(this).is(":checked")) {
              $(label)
                .closest(".leaflet-layerstree-node")
                .find("input")
                .not($(this))
                .each(function () {
      ...
      if (tree.hide) L.DomUtil.addClass(opened, hide)
      else L.DomUtil.addClass(closed, hide)
      if (noShow) {
        L.DomUtil.addClass(header, this.cls.neverShow)
        L.DomUtil.addClass(children, this.cls.childrenNopad)
      }
      $(input).attr("id", tree.name)
      $(label).attr("for", tree.name)

      $(input).prependTo($(entry))

      if (tree.layer && tree.layer.wmsParams && tree.layer._url) {
        //console.log(tree.layer._url)
        let nl = tree.layer.wmsParams.layers
        let url_img =
          tree.layer._url +
          "?request=GetLegendGraphic&format=image/png&mode=legendicon&icon=" +
          nl
        img = $(
          '<img style="display:none" class="legend-img" title="Легенда слоя" src="' +
            url_img +
            '">',
        ).prependTo($(name))
        img
      ...
      if (tree.lyr && tree.lyr.has_download && tree.lyr.db_is_valid) {
        //console.log(tree.lyr)
        $(
          '<i title="Скачать слой" class="btn-download fa fa-download" aria-hidden="true" data-id="' +
            tree.lyr.id +
            '"></i>',
        ).appendTo($(name))
      }
    ...
    _createCheckboxElementGroup: function (checked) {
      var input = document.createElement("input")
      input.type = "checkbox"
      input.className = "leaflet-control-layers-group-selector"
      input.defaultChecked = checked
      return input
    },
  })
